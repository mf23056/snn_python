...
class LIF:
    def __init__(self, dt=0.01, tau_m=20, V_rest=-65.0, R=10.0, I_back=0.0, V_reset=-65.0, V_th=-50.0, V_ref=5):
        self.dt = dt
        self.tau_m = tau_m
        self.V_rest = V_rest
        self.R = R
...
class Guetig_STDP:
    ...
    def __call__(self, delta_t):
        ...
        pos_mask = delta_t > 0
        neg_mask = delta_t <= 0
        delta_w[pos_mask] = self.A_plus * (torch.exp(-delta_t[pos_mask] / self.tau_plus) - 
                                           self.alpha * torch.exp(-delta_t[pos_mask] / (self.alpha * self.tau_plus)))
        delta_w[neg_mask] = -self.A_minus * (torch.exp(delta_t[neg_mask] / self.tau_minus) - 
                                             self.alpha * torch.exp(delta_t[neg_mask] / (self.alpha * self.tau_minus)))
        return delta_w
...
class SNN:
    ...
    def _initialize_synapses(self):
        ...
        weight_bias = torch.zeros((self.n_total, self.n_total), device=self.device)

        synapse_prob = torch.zeros((self.n_total, self.n_total), device=self.device)
        for pre_type, post_type, key, factor in [
            (1, 1, 'EE', 0.16),
            (1, 0, 'EI', 0.25),
            (0, 1, 'IE', 0.38),
            (0, 0, 'II', 0.10)]:
            mask = (self.neuron_types.unsqueeze(1) == pre_type) & (self.neuron_types == post_type)
            prob = factor * torch.exp(-distances[mask] ** 2 / (1 / (factor * math.sqrt(math.pi))) ** 2)
            synapse_prob[mask] = prob
            weight_bias[mask] = 1 if pre_type == 1 else -1

        random_vals = torch.rand_like(synapse_prob)
        self.weights = (synapse_prob > random_vals).float() * weight_bias
...
if __name__ == '__main__':
    ...
    spike_record = network.run_simulation(T=2000)
